const multporn = new MultpornAPI()

function multpornChangePage(page, makeNewPage, updateTabIndex) {
	page = page || 1
	if (updateTabIndex == null) updateTabIndex = true
	let id
	if (makeNewPage) {
		id = createNewTab(`multpornChangePage(${page}, 0, false)`, true, 0)
		...
		if (updateTabIndex == true) tabs[thisTagIndex].addHistory(`multpornChangePage(${page}, 0, false)`)
	}
	createNewMultpornTab(id, page)
}

function createNewMultpornTab(id, pageNumber) {
	if (id == null) { PopAlert(defaultSettingLang.tab_at_limit, 'danger'); return }
	...
	multporn.getPage({page:pageNumber, random:true, category:true}, (err, result) => {
		...
		tabs[thisTabIndex].icon.setAttribute('src', 'Image/sites/multporn-30x30.jpg')
		tabs[thisTabIndex].ClearLinks()
		let valueStorage, html = '<div>'
		...
		for (let i = 0; i < result.content.length; i++) html += `<div onmousedown="LinkClick(${thisTabIndex},${tabs[thisTabIndex].AddLink(2, result.content[i].id)},this)" cid="${result.content[i].id}" h="0"><img src="${result.content[i].thumb}"${setting.lazy_loading ? ' loading="lazy"' : ''}>${result.content[i].pages != null ? '<span>'+result.content[i].pages+'</span>' : ''}<p>${result.content[i].title}</p>${Downloader.IsDownloading(0, result.content[i].id) ? '<cid><img class="spin" src="Image/dual-ring-success-'+wt_fps+'.gif"></cid>' : '<button onclick="multpornDownloader(this.parentElement.getAttribute(\'cid\'))">Download</button>'}</div>`
...
function multpornOpenPost(makeNewPage, id, updateTabIndex) {
	let pageId, thisTabIndex
	if (updateTabIndex == null) updateTabIndex = true
	if (makeNewPage) {
		pageId = createNewTab(`multpornOpenPost(0, '${id}', false)`, true, 0)
		...
		if (updateTabIndex == true) tabs[thisTabIndex].addHistory(`multpornOpenPost(0, '${id}', false)`)
	}
	...
	multporn.getComic(id, {}, (err, result) => {
		...
		else html += `<div class="browser-comic-have" sssite="0" ccid="${id}"><button onclick="multpornDownloader('${id}')">Download</button><button class="add-to-have" onclick="AddToHave(0, '${id}')">Add To Have</button></div>`
		...
		const afterImage = () => {
			...
			if (result.related != undefined) {
				...
				for (let i = 0; i < result.related.length; i++) html += `<div onmousedown="LinkClick(${thisTabIndex},${tabs[thisTabIndex].AddLink(2, result.related[i].id)},this)" cid="${result.related[i].id}" h="0"><img src="${result.related[i].thumb}"${setting.lazy_loading ? ' loading="lazy"' : ''}>${result.related[i].pages != null ? '<span>'+result.related[i].pages+'</span>' : ''}<p>${result.related[i].title}</p>${Downloader.IsDownloading(0, result.related[i].id) ? '<cid><img class="spin" src="Image/dual-ring-success-'+wt_fps+'.gif"></cid>' : '<button onclick="multpornDownloader(this.parentElement.getAttribute(\'cid\'))">Download</button>'}</div>`
				html += '</div></div>'
			}
...
function multpornOpenCategory(name, page, shortName, makeNewPage, updateTabIndex) {
	name = name || null
	page = page || 1
	...
	if (makeNewPage) {
		pageId = createNewTab(`multpornOpenCategory('${name}', ${page}, '${shortName}', 0, false)`, true, 0)
		if (pageId == null) { PopAlert(defaultSettingLang.tab_at_limit, 'danger'); return }
		thisTabIndex = GetTabIndexById(pageId)
	} else {
		thisTabIndex = GetTabIndexById(pageId)
		...
		if (updateTabIndex == true) tabs[thisTabIndex].addHistory(`multpornOpenCategory('${name}', ${page}, '${shortName}', 0, false)`)
	}
	...
	tabs[thisTabIndex].icon.setAttribute('src', `Image/dual-ring-primary-${wt_fps}.gif`)
	tabs[thisTabIndex].page.innerHTML = `<div class="browser-page-loading"><img class="spin" style="width:60px;height:60px" src="Image/dual-ring-primary-${wt_fps}.gif"><p>Loading...</p></div>`
	multporn.getCategory(name, {page:page, random:true, category:true}, (err, result) => {
		if (document.getElementById(pageId) == undefined) return
		tabs[thisTabIndex].ir = false
		checkBrowserTools(thisTabIndex)
		tabs[thisTabIndex].page.innerHTML = null
		...
		// Random
		html += '</div><div class="xlecx-post-container">'
		for (let i = 0; i < result.random.length; i++) html += `<div onmousedown="LinkClick(${thisTabIndex},${tabs[thisTabIndex].AddLink(2, result.random[i].id)},this)" cid="${result.random[i].id}" h="0"><img src="${result.random[i].thumb}"${setting.lazy_loading ? ' loading="lazy"' : ''}>${result.random[i].pages != null ? '<span>'+result.random[i].pages+'</span>' : ''}<p>${result.random[i].title}</p>${Downloader.IsDownloading(0, result.random[i].id) ? '<cid><img class="spin" src="Image/dual-ring-success-'+wt_fps+'.gif"></cid>' : '<button onclick="multpornDownloader(this.parentElement.getAttribute(\'cid\'))">Download</button>'}</div>`
...
function multpornOpenTagContentMaker(result, thisTabIndex, name, whitch) {
	let html = ''
...
function multpornOpenTag(name, page, whitch, makeNewPage, updateTabIndex) {
	let pageId, thisTabIndex
	name = name || null
	page = page || 1
	whitch = whitch || 1
	...
	if (updateTabIndex == null) updateTabIndex = true
	if (makeNewPage) {
		pageId = createNewTab(`multpornOpenTag('${name}', ${page}, ${whitch}, 0, false)`, true, 0)
		if (pageId == null) { PopAlert(defaultSettingLang.tab_at_limit, 'danger'); return }
		thisTabIndex = GetTabIndexById(pageId)
	} else {
		pageId = activeTabComicId
		...
		if (updateTabIndex == true) tabs[thisTabIndex].addHistory(`multpornOpenTag('${name}', ${page}, ${whitch}, 0, false)`)
	}
	...
	switch (whitch) {
		case 1:
			multporn.getGroup(name, {page:page, category:true}, (err, result) => {
				if (document.getElementById(pageId) == undefined) return
				tabs[thisTabIndex].ir = false
				checkBrowserTools(thisTabIndex)
				tabs[thisTabIndex].page.innerHTML = null
				...
				multpornOpenTagContentMaker(result, thisTabIndex, name, whitch)
			})
			break
		case 2:
			multporn.getArtist(name, {page:page, category:true}, (err, result) => {
				if (document.getElementById(pageId) == undefined) return
				tabs[thisTabIndex].ir = false
				checkBrowserTools(thisTabIndex)
				tabs[thisTabIndex].page.innerHTML = null
			...
			break
		case 3:
			multporn.getParody(name, {page:page, category:true}, (err, result) => {
				if (document.getElementById(pageId) == undefined) return
				tabs[thisTabIndex].ir = false
				checkBrowserTools(thisTabIndex)
				tabs[thisTabIndex].page.innerHTML = null
			...
			break
		case 4:
			multporn.getTag(name, {page:page, category:true}, (err, result) => {
				if (document.getElementById(pageId) == undefined) return
				tabs[thisTabIndex].ir = false
				checkBrowserTools(thisTabIndex)
				tabs[thisTabIndex].page.innerHTML = null
...
function multpornSearch(text, page, whitchbutton, updateTabIndex) {
	...
	if (makeNewPage) {
		pageId = createNewTab(`multpornSearch('${text}', ${page}, 0, false)`, true, 0)
		if (pageId == null) { PopAlert(defaultSettingLang.tab_at_limit, 'danger'); return }
		thisTabIndex = GetTabIndexById(pageId)
	} else {
		pageId = activeTabComicId
		...
		if (updateTabIndex == true) tabs[thisTabIndex].addHistory(`multpornSearch('${text}', ${page}, 0, false)`)
	}
	...
	tabs[thisTabIndex].icon.setAttribute('src', `Image/dual-ring-primary-${wt_fps}.gif`)
	tabs[thisTabIndex].page.innerHTML = `<div class="browser-page-loading"><img class="spin" style="width:60px;height:60px" src="Image/dual-ring-primary-${wt_fps}.gif"><p>Loading...</p></div>`
	multporn.search(text, {page:page, category:true}, (err, result) => {
		if (document.getElementById(pageId) == undefined) return
		tabs[thisTabIndex].ir = false
		checkBrowserTools(thisTabIndex)
		tabs[thisTabIndex].page.innerHTML = null
...
function multpornOpenAllTags(makeNewPage, updateTabIndex) {
	let pageId, thisTabIndex
	if (updateTabIndex == null) updateTabIndex = true
	if (makeNewPage) {
		pageId = createNewTab('multpornOpenAllTags(0, false)', true, 0)
		if (pageId == null) { PopAlert(defaultSettingLang.tab_at_limit, 'danger'); return }
		thisTabIndex = GetTabIndexById(pageId)
	} else {
		pageId = activeTabComicId
		...
		if (updateTabIndex == true) tabs[thisTabIndex].addHistory('multpornOpenAllTags(0, false)')
	}
	...
	tabs[thisTabIndex].icon.setAttribute('src', `Image/dual-ring-primary-${wt_fps}.gif`)
	tabs[thisTabIndex].page.innerHTML = `<div class="browser-page-loading"><img class="spin" style="width:60px;height:60px" src="Image/dual-ring-primary-${wt_fps}.gif"><p>Loading...</p></div>`
	multporn.getAllTags(true, (err, result) => {
		if (document.getElementById(pageId) == undefined) return
		tabs[thisTabIndex].ir = false
		checkBrowserTools(thisTabIndex)
		tabs[thisTabIndex].page.innerHTML = null
...
function multpornJumpPage(index, page) {
	switch (index) {
		case 0:
		...
		case 1:
			searchTimer = setTimeout(() => {
				multpornChangePage(page, false)
			}, 185)
			break
		case 2:
			searchTimer = setTimeout(() => {
				multpornOpenCategory(tabs[activeTabIndex].options[0], page, tabs[activeTabIndex].options[1], false)
			}, 185)
			break
		case 3:
			searchTimer = setTimeout(() => {
				multpornOpenTag(tabs[activeTabIndex].options[0], page, tabs[activeTabIndex].options[1], false)
			}, 185)
			break
	}
}

function multpornDownloader(id) {
	...
	multporn.getComic(id, {related:false}, (err, result) => {
		...
		Downloader.Add(index, result.url, multporn.baseURL+'/'+result.images[0].thumb, downloadImageList, sendingResult)
	})
}

function multpornRepairComicInfoGetInfo(id, whitch) {
		...
		switch (whitch) {
			...
			case 5:
				...
				for (let i = 0; i < need_repair.length; i++) {
					const thumb = multporn.baseURL+neededResult[need_repair[i][1]].thumb
					newList.push(thumb)
...
function multpornRepairAllComicInfo(id, comic_id) {
	multporn.getComic(id, {related:false}, (err, result) => {
		if (err) {
